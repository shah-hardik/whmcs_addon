...
{/if}
  ...
  {if $totalrecurringmonthly || $totalrecurringquarterly || $totalrecurringsemiannually || $totalrecurringannually || $totalrecurringbiennially || $totalrecurringtriennially}
   ...
   {if $product.productinfo.pid eq "29"}
   <br>
   <br /><tr class="recurring">
   <td align="right" style="font-size:11px; font-weight: normal; background-color:white !important" colspan="2">{$product.productinfo.name} plan is free for first month. Subsequent months will be automatically billed unless you send a cancelation email within 30 days after signing up!<br>Must provide accurate physical address and contact information.</td>
   </tr>
   {else}
    <tr class="recurring">
...
{/if}
      ...
      <input style="font-size:10px;padding:3px;" type="button" value="{$LANG.continueshopping}" onclick="window.location='cart.php'" />
